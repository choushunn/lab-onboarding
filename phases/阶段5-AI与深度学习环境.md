...
| 子项 | 任务 | 验收 |
|------|------|------|
| 5.1 NVIDIA 驱动与 CUDA | 安装 NVIDIA 显卡驱动；按 [CUDA 官方文档](https://developer.nvidia.com/cuda-downloads) 安装 CUDA 工具包；安装 cuDNN；验证 `nvidia-smi` 输出 GPU 信息 | `nvidia-smi` 显示 GPU 型号与驱动版本；`nvcc --version` 输出 CUDA 版本 |
| 5.2 PyTorch 环境 | 创建 Conda 环境；按 [PyTorch 官网](https://pytorch.org/) 命令安装 PyTorch CUDA 版；运行 `torch.cuda.is_available()` 验证 GPU 可用 | `import torch; torch.cuda.is_available()` 返回 `True` |
